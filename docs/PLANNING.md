...
| ID   | Story | Acceptance Criteria |
|------|-------|---------------------|
| HU13 [done] | As a user, I can check if my email was exposed in breaches (opt-in) | Email field on index.html — result appears inline in data_exposure dimension |
...
| HU30 [done] | As a user, each scan is persisted with the full payload | `scans` table contains raw_payload and result_json |
...
| HU33 [done] | As a user, I configure API keys via `.env` file | `HIBP_API_KEY` and `ABUSEIPDB_API_KEY` loaded via python-dotenv |
