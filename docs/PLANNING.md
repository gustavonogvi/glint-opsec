...
|------|-------|---------------------|
| HU1  | As a user, when I open the tool, my canvas fingerprint is collected automatically | `canvas_hash` present in the payload sent to the backend |
| HU2  | As a user, my AudioContext fingerprint is collected | `audio_hash` present in the payload |
| HU3  | As a user, my installed font list is enumerated | `fonts[]` detected, `font_groups[]` with software signatures (ms_office, adobe_cc, coding, etc.), `fonts_blocked` flag if browser normalizes measurements |
| HU4  | As a user, `navigator` properties are collected | UA, language, platform, hardwareConcurrency, timezone present |
| HU5  | As a user, screen vs. window geometry is captured | `screen_width`, `window_inner_width` are distinct (when toolbar is present) |
| HU6  | As a user, WebRTC attempts to collect my real IP | `webrtc.local_ips[]` populated if WebRTC is not blocked |
| HU7  | As a user, WebGL vendor/renderer is collected | `webgl.vendor` and `webgl.renderer` present |
| HU8  | As a system, all collectors run in parallel and the payload is sent to the backend | `Promise.all()` → POST in < 3s |
