...
export async function collectFingerprint(onProgress) {
    ...
    const [canvas, audio, fonts, navigator, screen, webrtc, webgl] = await Promise.all([
        track("canvas",    collectCanvas()),
        track("audio",     collectAudio()),
        track("fonts",     collectFonts()),
        track("navigator", collectNavigator()),
        track("screen",    collectScreen()),
        track("webrtc",    collectWebRTC()),
        track("webgl",     collectWebGL()),
    ]);
